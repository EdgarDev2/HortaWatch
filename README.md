...
2. Clonar el repositorio en "C:\wamp64\www":
   ...
   ```
3. 3. Cambiar de directorio al proyecto e inicializarlo seleccionando 0
   ```bash
   cd HortaWatch && php init
   ```
4. Configurar el nombre de la Base de datos a "sistemariego" en common/main-local.php en la linea:
   ```bash
   'dsn' => 'mysql:host=localhost;dbname=sistemariego',
   ```
5. Instalar las dependencias externas
   ```sql
   composer install
   ```
6. Crear la base de datos con MySQL console de WampServer
   ```sql
   CREATE DATABASE sistemariego CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
   ```
7. Selecciona la base de datos
   ```sql
   USE sistemariego;
   ```
